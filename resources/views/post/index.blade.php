...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex gap-3">
            <h5>Post List</h5>
            <a href="{{route('create-post')}}" class="btn btn-sm btn-ex-blue"
                                ...
                                <div class="d-flex justify-content-between align-items-end">
                                    ...
                                    <div>
                                        {{--<button
                                            ...
                                            <span class="fw-bold fs-8">{{$post->menu ? "-":"+"}}</span>Menu
                                        </button>--}}
                                        ...
                                        <a href="{{route('edit-post', $post->id)}}" class="btn btn-success btn-sm"
                    ...
                    <tr>
                        ...
                        <td>{{$post->title}}
                            ...
                            @else
                                <span class="badge text-bg-warning">Inactive</span>
                            @endif</td>
                        <td><img src="{{asset("storage/media/".$post->image)}}" class="img-thumbnail"
                                 style="height: 160px"></td>
                        <td>{{$post->seofield?$post->seofield->excerpt:""}}</td>
                        <td>{{$post->created_at}}</td>
                        <td>
                            @if($post->menu == null)
                                <a href="{{route('add-to-menu', $post->id)}}" class="btn badge btn-dark btn-sm p-2">
                                    <span class="fw-bold fs-8">+</span> Menu
                                </a>
                            @else
                                <a href="{{route('remove-from-menu', $post->id)}}"
                                   class="btn badge btn-secondary btn-sm p-2">
                                    <span class="fw-bold fs-8">-</span> Menu
                                </a>
                            @endif

                            @if($post->notification == null)
                                <a href="{{route('add-to-notification', $post->id)}}"
                                   class="btn badge btn-dark btn-sm p-2">
                                    <span class="fw-bold fs-8">+</span>
                                    <i class="fa-solid fa-bell"></i>
                                </a>
                            @else
                                <a href="{{route('remove-from-notification', $post->id)}}"
                                   class="btn badge btn-secondary btn-sm p-2">
                                    <span class="fw-bold fs-8">-</span>
                                    <i class="fa-solid fa-bell"></i>
                                </a>
