<script>
    ...
    $(".features").click(function () {
        ...
        $("#submitbtn").hide()
        let id = $(this).data("id");
        $("#cancelBtn").data("test-id", id);
        $("#cancelBtn").click()
        $("#formBtn").data("test-id", id);
        let featureAjaxURL = "{{route("featureAjax",[":id", "test"])}}";
            ...
            success: function (result) {
                ...
                for (r in result) {
                    data += `<tr class="datarow"><td>${result[r]["id"]}</td><td>${result[r]["title"]}</td>
                                    <td>
                                        <a class="btn btn-success btn-sm edit-feature" data-test-id="${id}" data-feature-id="${result[r]["id"]}" data-hi="${result[r]["translations"][1]["title"]}" data-en="${result[r]["translations"][0]["title"]}"  >
                                            <i class="fa-regular fa-pen-to-square"></i>
                                        </a>
                                        <button type="button" class="btn btn-danger btn-sm feature-del-btn"
                                                data-feature-id="${result[r]["id"]}" data-test-id="${id}"  data-bs-toggle="tooltip" data-bs-title="Delete Feature">
    ...
    $(document).on('click', "#formBtn", function () {
        ...
        let createURL = "{{route('createFeatureAjax', ":test-id")}}";
        featureAjaxURL = createURL.replace(":test-id", id)
        $.ajax({
            url: featureAjaxURL,
            type: "POST",
            ...
            success: function (result) {
                data = `<tr class="datarow"><td>${result["id"]}</td><td>${result["title"]}</td>
                                    <td>
                                        <a class="btn btn-success btn-sm edit-feature" data-test-id="${id}" data-feature-id="${result["id"]}" data-hi="${result["translations"][1]["title"]}" data-en="${result["translations"][0]["title"]}" data-bs-toggle="tooltip" data-bs-title="Edit Feature" >
                                            <i class="fa-regular fa-pen-to-square"></i>
                                        </a>
                                        <button type="button" class="btn btn-danger btn-sm feature-del-btn"
                                                data-feature-id="${result["id"]}" data-test-id="${id}"  data-bs-toggle="tooltip" data-bs-title="Delete Feature">
                                            <i class="fa-regular fa-trash-can"></i>
                                        </button>
                                    </td></tr>`;
